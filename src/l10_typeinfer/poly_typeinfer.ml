...
(** Raised when inference fails due to incompatible equations or unbound
    identifiers. *)

(* Type representation *******************************************************)

type type_var = int
(** Integer identifiers for type variables, later printed as ['a], ['b], ... *)

type typ = TInt | TBool | TFun of typ * typ | TVar of type_var
(** Types: integers, booleans, arrows, or variables. *)

type type_scheme = Forall of type_var list * typ
(** Polymorphic type schemes quantify a list of variables over a monotype. *)

type type_env = (string * type_scheme) list
(** Type environments map term variables to schemes. *)

type substitution = (type_var * typ) list
(** Substitutions equate type variables with concrete types. *)

(* Fresh type variables ******************************************************)

(** [fresh_int ()] returns an integer identifier that is unique for every call
    during a single run. This acts as the "name supply" in Algorithm W. Example:
    calling it three times produces the sequence 0, 1, 2, which are later
    rendered as the schematic type variables `'a0`, `'a1`, ... *)
let fresh_int =
...
(** [collect_type_vars ty acc] gathers type variables in [ty] following their
    order of appearance so that we can assign prettified names
    deterministically. *)
...
(** [string_of_type ty] produces an OCaml-style string using the naming scheme
    ...
    Example: [string_of_type (TFun (TVar 2, TFun (TVar 0, TVar 2)))] prints
    ["'a -> 'b -> 'a"]. *)
let string_of_type ty =
  ...
  let rec to_string = function
    ...
    | TBool -> "bool"
    | TVar v -> (
        match List.assoc_opt v mapping with Some name -> name | None -> "'_")
    | TFun (t1, t2) ->
        let lhs =
          match t1 with
...
(** [apply_subst_type subst ty] walks over [ty] and replaces the free variables
    ...
    [subst = [ (0, TInt); (1, TBool) ]], applying to [TFun (TVar 0, TVar 2)]
    produces [TFun (TInt, TVar 2)]. Note how `'a2 remains because it does not
    appear in [subst]. *)
...
(** [apply_subst_env subst env] rewrites every scheme in the environment with
    ...
    new equalities. Example: if [env] binds [x] to [Forall ([], TVar 0)] and
    [subst] forces [TVar 0] to [TInt], then [apply_subst_env subst env] binds
    [x] to [Forall ([], TInt)]. *)
...
(** [compose_subst s2 s1] applies [s2] after [s1]. Operationally, we first push
    ...
    [s2 = [ (1, TInt) ]] yields [ [ (1, TInt); (0, TInt) ] ], which when applied
...
(** [free_type_vars_type ty] collects the free type variables of [ty] as a set,
    used when deciding which variables may be generalized. *)
...
(** [generalize env ty] abstracts the type variables in [ty] that are not
    ...
    Example: with [env] containing only
    [("id", Forall ([], TFun (TVar 0, TVar 0)))] and
    [ty = TFun (TVar 1, TVar 1)], the result is
...
(** [instantiate scheme] replaces each quantified variable with a fresh type
    ...
    unrelated types [TFun (TVar 3, TVar 3)] and [TFun (TVar 5, TVar 5)],
    allowing the same [id] function to be used at different types. *)
...
(** [occurs v ty] checks whether the type variable [v] appears inside [ty]. This
    is the "occurs check" that prevents us from constructing infinite types,
    such as trying to solve `'a = 'a -> 'b`. *)
...
(** [bind_variable v ty] links the variable [v] with [ty] in the current
    ...
    substitution; otherwise we ensure the occurs check passes. Example: binding
    [v = 0] with [ty = TInt] yields the substitution [ [ (0, TInt) ] ], but
    binding [v = 0] with [ty = TFun (TVar 0, TInt)] raises [TypeError] because
    `'a0` would appear on both sides. *)
...
(** [unify t1 t2] determines the most general substitution that makes [t1] and
    ...
    [unify (TFun (TVar 0, TInt)) (TFun (TBool, TVar 1))] returns the
    substitution [ [ (1, TInt); (0, TBool) ] ]. Applying that substitution to
    both inputs yields the identical type [TFun (TBool, TInt)]. *)
...
(** [lookup env name] retrieves the type scheme associated with [name]. Example:
    if [env = [ ("x", Forall ([], TInt)) ]], then [lookup env "x"] returns
    [Forall ([], TInt)], while looking up ["y"] raises a [TypeError] describing
...
(** [infer_expr env e] implements Algorithm W and returns a pair consisting of
    ...
    - For [If (Bool true, Int 0, Bool false)] the call to [unify] on the
      branches raises [TypeError] because the branches do not agree in type. The
      function composes substitutions in the order demanded by the algorithm so
      that later inferences see earlier equalities. *)
let rec infer_expr (tenv : type_env) (e : expr) : substitution * typ =
  ...
  | App (e1, e2) ->
      ...
      let s_unify = unify (apply_subst_type s2 t1) (TFun (t2, result_type)) in
