...
- subsitute expr `v` for var `x` in (body) expr `e`; i.e., [`v`/`x`]`e`
...
- driven by `subst` --- this approach to evaluation is known as the
  *substitution model*
...
E.g., for the lambda calculus, here are the rules in BNF (they are a bit more
complicated than the BNF we saw earlier, to account for left-associativity of
...
Parsers recognize *context-free grammars* using *pushdown automata* (automata
with a stack for memory).
...
The `LLVM` project is a popular compiler toolchain designed around a
language-independent IR.

- Front-ends for the LLVM IR exist for C, C++, C#, Haskell, Java, ML,
  Objective-C, Ruby, Rust, Scala, Swift, etc.
