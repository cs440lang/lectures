...
E.g., `bool` = `bool`; `ɑ` = `int`; `β→ int` = `bool→ ɣ`
...
I.e., `e` has type `τ` under `Γ` iff all constraints in `C` are satisfiable.
...
To solve a list of constraints {`τ₁` = `τ₂`, ... },
